...
## 개요
Spring Boot 앱을 **코드 수정 없이** 계측하는 Observability SDK. ByteBuddy로 런타임 바이트코드를 계측해 HTTP/LOG/JDBC/METHOD_TRACE/@LogEvent 이벤트를 NATS JetStream으로 전송한다.

## 핵심 스택
- Kotlin 2.3.20 / JVM 21
- ByteBuddy 1.15.11 / jnats 2.17.0 / Protobuf 3.25.3
...
├── BatchTransporter.kt             # Event Queue(10k) + NATS JetStream lazy init
...
| 환경변수/프로퍼티 | 기본값 | 설명 |
|---|---|---|
| `LOGFRIENDS_NATS_URL` | `nats://localhost:4222` | NATS 서버 URL |
| `logfriends.nats.url` | `nats://localhost:4222` | NATS 서버 URL (프로퍼티) |
| `logfriends.nats.subject` | `log-friends.batch` | JetStream subject |
...
- IDE: **VSCode** (멀티 루트 워크스페이스 — 4개 레포 동시 작업)
- Claude Code: VSCode 익스텐션 사용

## 주의사항

- 필수 JVM 옵션: `-Djdk.attach.allowAttachSelf=true`
- NATS Connection lazy 초기화 (SLF4J 로드 순서 보장)
